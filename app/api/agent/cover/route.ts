...
export async function POST(request: Request) {
  try {
    ...
    const { data: postRow } = await admin
      .from("posts")
      .select("author_id, primary_locale")
      .eq("id", post_id)
      .maybeSingle();
    if (postRow?.author_id) {
      ...
      if (clientRow) {
        ...
        omitOnImageText = clientDisallowsCoverOnImageText(customInstructions);
    ...
    const headlineForCover = omitOnImageText ? "" : query.trim().split(/\s+/).slice(0, 4).join(" ");
    ...
    try {
      if (refParts.length > 0) {
        ...
        if (!referenceVisionBrief?.trim()) {
          console.warn("[cover] reference vision unavailable — generating cover from reference images only");
        }
      }
    ...
    try {
      ...
      if (!picsumRes.ok) {
        const msg = imgErr instanceof Error ? imgErr.message : "Cover generation failed";
