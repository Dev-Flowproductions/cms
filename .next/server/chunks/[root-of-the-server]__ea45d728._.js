...
const imagenAI = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$google$2f$genai$2f$dist$2f$node$2f$index$2e$mjs__$5b$app$2d$route$5d$__$28$ecmascript$29$__["GoogleGenAI"]({
...
async function POST(request) {
    ...
    try {
        ...
        try {
            const coverPrompt = `Professional hero image for a blog post about: "${generated.focus_keyword}". ` + `Wide landscape format, 16:9 ratio. High quality, modern, editorial photography style. ` + `Clean composition, no text overlays, no watermarks.`;
            ...
            if (img?.imageBytes) {
                const binaryStr = atob(img.imageBytes);
                const bytes = new Uint8Array(binaryStr.length);
                for(let i = 0; i < binaryStr.length; i++)bytes[i] = binaryStr.charCodeAt(i);
                const coverPath = `${post_id}/cover-${Date.now()}.jpg`;
                const { error: uploadErr } = await admin.storage.from("covers").upload(coverPath, bytes.buffer, {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__ea45d728._.js.map