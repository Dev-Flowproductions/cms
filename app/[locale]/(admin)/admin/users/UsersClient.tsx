        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
          {users.map((u) => {
            ...
            return (
              <div
                key={u.id}
                className="admin-shell-glass rounded-2xl overflow-hidden flex flex-col"
              ...
              >
                ...
                >
                  <div className="flex items-center gap-2.5 min-w-0">
                    <span
                      ...
                      style={{
                        background: "rgba(104, 57, 234, 0.12)",
                        border: "1px solid rgba(104, 57, 234, 0.22)",
                    ...
                    <div className="min-w-0">
                      {accountName ? (
                        <div className="text-xs font-semibold truncate" style={{ color: "var(--adm-on-surface)" }}>
                          {accountName}
                        </div>
                      ) : null}
                      <div className="text-xs truncate" style={{ color: "var(--adm-on-variant)" }}>
                        {u.email}
                      </div>
                    </div>
                  </div>
                  <button
                    ...
                    onMouseEnter={(e) => {
                      if (!isDeleting && !isPending) {
                        e.currentTarget.style.background = "rgba(255, 180, 171, 0.16)";
                        e.currentTarget.style.borderColor = "var(--adm-error)";
                      }
                    }}
                    onMouseLeave={(e) => {
                ...
                <div className="p-4 grid grid-cols-2 gap-x-4 gap-y-3 text-xs">
                  <div>
                    <p className="text-[10px] font-semibold uppercase tracking-wider mb-0.5" style={{ color: "var(--adm-on-variant)" }}>{t("colDomain")}</p>
                    {onboardingPending ? (
                      ...
                      <span className="font-mono truncate block" style={{ color: "var(--adm-on-surface)" }} title={u.domain ?? undefined}>{u.domain ?? "—"}</span>
                    )}
                  </div>
                  <div>
                    <p className="text-[10px] font-semibold uppercase tracking-wider mb-0.5" style={{ color: "var(--adm-on-variant)" }}>{t("colFrequency")}</p>
                    <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-xs font-medium" style={{ background: "rgba(104, 57, 234, 0.1)", color: "var(--adm-primary)" }}>
                      <span className="w-1.5 h-1.5 rounded-full" style={{ background: "var(--adm-primary)" }} />
                      {FREQUENCY_LABELS[u.frequency] ?? u.frequency}
                    </span>
                  </div>
                  <div>
                    <p className="text-[10px] font-semibold uppercase tracking-wider mb-0.5" style={{ color: "var(--adm-on-variant)" }}>{t("colNextPost")}</p>
                    <span className="font-medium" style={{ color: getNextPostLabel(u) === t("nextPostDueNow") ? "var(--adm-primary)" : "var(--adm-on-variant)" }}>
                      {getNextPostLabel(u)}
                    </span>
                  </div>
                  <div>
                    <p className="text-[10px] font-semibold uppercase tracking-wider mb-0.5" style={{ color: "var(--adm-on-variant)" }}>{t("colGoogle")}</p>
                    {googleConnected ? (
                      ...
                      <span style={{ color: "var(--adm-on-variant)" }}>—</span>
                    )}
                  </div>
                  <div className="col-span-2">
                    <p className="text-[10px] font-semibold uppercase tracking-wider mb-0.5" style={{ color: "var(--adm-on-variant)" }}>{t("colBrand")}</p>
                    <span className="truncate block" style={{ color: "var(--adm-on-variant)" }}>{u.company_name ?? u.brand_name ?? "—"}</span>
                  </div>
                  <div>
                    <p className="text-[10px] font-semibold uppercase tracking-wider mb-0.5" style={{ color: "var(--adm-on-variant)" }}>{t("colWebhook")}</p>
                    <button
                      type="button"
                      ...
                      style={{
                        background: hasWebhook ? "rgba(34,211,160,0.1)" : "var(--adm-surface-highest)",
                        color: hasWebhook ? "#22d3a0" : "var(--adm-on-variant)",
                        border: hasWebhook ? "1px solid rgba(34,211,160,0.25)" : "1px solid var(--adm-border-subtle)",
                      }}
                    >
                      {hasWebhook ? (
                        <>
                  ...
                  <div>
                    <p className="text-[10px] font-semibold uppercase tracking-wider mb-0.5" style={{ color: "var(--adm-on-variant)" }}>{t("colCreated")}</p>
                    <span style={{ color: "var(--adm-on-variant)" }}>
                  ...
                  <div className="px-4 pb-4 flex flex-col gap-2">
                    <button
                      ...
                      style={{
                        ...
                        color: "var(--adm-primary)",
                        border: "1px solid rgba(104, 57, 234, 0.35)",
                      }}
                    >
                    ...
                    {!u.custom_instructions?.trim() ? (
                      <button
                        ...
                        style={{
                          background: instructionsCreatingUserId === u.user_id ? "var(--adm-surface-highest)" : "rgba(104, 57, 234, 0.12)",
                          color: instructionsCreatingUserId === u.user_id ? "var(--adm-on-variant)" : "var(--adm-primary)",
                          border: "1px solid rgba(104, 57, 234, 0.35)",
                        }}
                      >
                    ...
                    {!u.custom_instructions?.trim() && instructionsError && instructionsErrorUserId === u.user_id && (
                      <p className="text-xs px-2" style={{ color: "var(--adm-error)" }}>{instructionsError}</p>
                    )}
                    {u.custom_instructions?.trim() ? (
                      <button
                        type="button"
                        ...
                        className="w-full flex items-center justify-center gap-2 py-2 rounded-xl text-xs font-semibold transition-all"
                        style={{
                          background: instructionsExpandedUserId === u.user_id ? "var(--adm-surface-highest)" : "rgba(245,158,11,0.12)",
                          color: instructionsExpandedUserId === u.user_id ? "var(--adm-primary)" : "#f59e0b",
                    ...
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-2 py-2 rounded-xl text-xs font-semibold transition-all disabled:opacity-50"
                      style={{
                        background: generatingUserId === u.user_id ? "var(--adm-surface-highest)" : "linear-gradient(135deg, #6839ea, #8b6bef)",
                        color: generatingUserId === u.user_id ? "var(--adm-on-variant)" : "#fff",
                        border: generatingUserId === u.user_id ? "1px solid var(--adm-border-subtle)" : "none",
                        boxShadow: generatingUserId === u.user_id ? "none" : "0 0 14px rgba(104, 57, 234, 0.35)",
                      }}
                    >
                ...
                {expandedEditUserId === u.user_id && (
                  <div className="px-4 pb-4" style={{ borderTop: "1px solid var(--adm-border-subtle)" }}>
                    <div className="rounded-xl mt-3 overflow-hidden" style={{ background: "var(--adm-surface-highest)", border: "1px solid var(--adm-border-subtle)" }}>
                      <EditUserConfig
                        user={u}
                ...
                {webhookExpanded && (
                  ...
                  >
                    <div className="rounded-xl p-4" style={{ background: "var(--adm-surface-highest)", border: "1px solid var(--adm-border-subtle)" }}>
                      <div className="flex items-center justify-between mb-3">
                        <p className="text-[10px] font-semibold uppercase tracking-widest" style={{ color: "var(--adm-on-variant)" }}>Publishing webhook</p>
                        <button type="button" onClick={() => setExpandedWebhook(null)} className="text-xs" style={{ color: "var(--adm-on-variant)" }}>
                          {t("webhookClose")}
                        </button>
                      </div>
                  ...
                  >
                    <div className="rounded-xl p-4" style={{ background: "var(--adm-surface-highest)", border: "1px solid var(--adm-border-subtle)" }}>
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <button type="button" onClick={() => setInstructionsExpandedUserId(null)} className="text-xs" style={{ color: "var(--adm-on-variant)" }}>
                          {t("instructionsClose")}
                        </button>
                      </div>
                      <p className="text-xs mb-2" style={{ color: "var(--adm-on-variant)" }}>{t("instructionsDescription")}</p>
                      <textarea
